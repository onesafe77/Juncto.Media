...
export default function ArticleCard({
  ...
  return (
    <div 
      className={`bg-white rounded-2xl border border-blue-gray/30 overflow-hidden hover:shadow-lg transition-shadow group flex ${isFeatured ? 'flex-col lg:flex-row' : 'flex-row p-4 sm:p-6'} min-w-0`}
      style={isFeatured ? { borderLeftWidth: '4px', borderLeftColor: config.color } : {}}
    >
      <Link to={`/workspace/article/${id}`} className={`${isFeatured ? 'w-full aspect-video lg:w-[38%] lg:aspect-auto lg:min-h-[240px]' : 'w-20 h-20 sm:w-24 sm:h-[75px] lg:w-32 lg:h-[90px] rounded-lg'} relative overflow-hidden shrink-0 block`}>
      ...
      <div className={`flex-1 min-w-0 ${isFeatured ? 'p-4 sm:p-6 lg:p-8' : 'pl-4 sm:pl-6'} flex flex-col justify-between relative`}>
        <div className="min-w-0">
          ...
          <Link to={`/workspace/article/${id}`} className="block min-w-0">
          ...
          {isFeatured && (
            <div className="flex flex-wrap gap-2 mb-6">
              {tags.map(tag => (
                <span key={tag} className="text-xs font-medium text-text-light bg-off-white px-2 py-1 rounded">
                  {tag}
                </span>
              ))}
            </div>
